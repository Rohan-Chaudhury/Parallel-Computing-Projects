...
void *sort_thread_task_parallel(void *task_data) {
    ...
    struct task_arguments *t_meta = task_data;
    ptr = (*t_meta).ptr;
    my_id = (*t_meta).i;
    np = (*t_meta).np;
...
void sort_list_parallel() {
    ...
    for (int i = 0; i < num_threads; i++) {
        struct task_arguments *t_data = malloc (sizeof (struct task_arguments));
        (*t_data).i = i;
        (*t_data).np = np;
        (*t_data).num_threads = num_threads;
        (*t_data).ptr = ptr;
        pthread_create(&thread_id[i], NULL, &sort_thread_task_parallel, (void *)t_data);
    }
